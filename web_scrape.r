...
for(i in hotelIds)){
  data <- apiCallNearbyHotel(hotelIds)
  ...
  hotelIds <- append(hotelIds, data$hotelId)
  hotelIds <- unique(hotelIds)
}

h = read_html("https://sg.trip.com/hotels/list?city=73")

hotel = h %>% html_nodes(".compressmeta-hotel-wrap")